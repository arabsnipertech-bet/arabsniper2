...
def scan_dates(
    ...
    if str(state.get("version", "")) != VERSION:
        log(f"♻️ Reset stato quote: versione {state.get('version', 'N/D')} -> {VERSION}")
        state = default_state()
...
XG_MATCHES_PER_TEAM = max(0, min(safe_int(os.getenv("ARAB_XG_MATCHES_PER_TEAM", 6), 6), MAX_TEAM_MATCHES))
XG_MAX_CALLS_PER_RUN = max(0, safe_int(os.getenv("ARAB_XG_MAX_CALLS_PER_RUN", 120), 120))
...
def run_market_audit(client: APIClient, target_date: str, excluded_countries: Sequence[str]) -> Dict[str, Any]:
    ...
    fixtures = [row for row in (payload or {}).get("response", []) or []
                ...
                and (row.get("league", {}) or {}).get("country") not in excluded
                and not is_blacklisted_league((row.get("league", {}) or {}).get("name"))]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ArabSniper Quote Engine V1.1")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--today", action="store_true", help="Scansiona il giorno corrente")
    mode.add_argument("--night", action="store_true", help="Scansione rolling da oggi")
    mode.add_argument("--date", type=str, help="Data iniziale YYYY-MM-DD")
