...
def get_team_recent_matches(client: APIClient, team_id: int, last: int = MAX_TEAM_MATCHES) -> List[Dict[str, Any]]:
    ...
    for fixture_row in (payload or {}).get("response", []) or []:
        if not fixture_allowed(fixture_row, ()):
            continue
    ...
    for day_index, target_date in enumerate(target_dates, start=1):
        ...
        for fixture_row in response_rows:
            ...
            if fixture.get("status", {}).get("short") != "NS":
                continue
            if not fixture_allowed(fixture_row, excluded):
                continue
        ...
        for fid, old_item in existing_by_id.items():
            ...
            if not output_fixture_allowed(old_item, excluded):
        ...
        if not refreshed_matches and existing_matches:
            ...
            refreshed_matches = [
                ...
                for item in existing_matches
                if output_fixture_allowed(item, excluded)
            ]
...
def run_market_audit(client: APIClient, target_date: str, excluded_countries: Sequence[str]) -> Dict[str, Any]:
    ...
    fixtures = [row for row in (payload or {}).get("response", []) or []
                ...
                and fixture_allowed(row, excluded)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="ArabSniper Quote Engine V1.1.7")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--today", action="store_true", help="Scansiona il giorno corrente")
    mode.add_argument("--night", action="store_true", help="Scansione rolling da oggi")
    mode.add_argument("--date", type=str, help="Data iniziale YYYY-MM-DD")
...
# V1.1.7: livelli inferiori riconosciuti per paese. Serve a evitare che una
# lega minore con un nome non presente nei token generici venga promossa
# erroneamente a top tier soltanto perche' appartiene a un paese affidabile.
COUNTRY_LOWER_TIER_TOKENS: Dict[str, Tuple[str, ...]] = {
    "england": ("league one", "league two", "national league", "isthmian", "southern league", "northern premier", "fa trophy", "fa vase"),
    "italy": ("serie d", "eccellenza", "promozione", "campionato primavera"),
    "spain": ("primera federacion", "segunda federacion", "tercera federacion", "segunda division rfef", "tercera division"),
    "germany": ("3. liga", "3 liga", "regionalliga", "oberliga", "bayernliga", "verbandsliga", "landesliga"),
    "france": ("championnat national", "national 2", "national 3"),
    "netherlands": ("tweede divisie", "derde divisie", "vierde divisie"),
    "portugal": ("liga 3", "campeonato de portugal"),
    "belgium": ("national division 1", "division 1 amateur", "first amateur", "second amateur"),
    "scotland": ("league one", "league two", "highland league", "lowland league"),
    "turkey": ("2. lig", "3. lig"),
    "brazil": ("serie c", "serie d"),
    "argentina": ("federal a", "primera b metropolitana", "primera c", "primera d", "torneo regional"),
    "usa": ("mls next pro", "usl league one", "nisa"),
    "mexico": ("liga premier", "liga tdp"),
    "austria": ("2. liga", "regionalliga", "landesliga"),
    "switzerland": ("challenge league", "promotion league", "1. liga classic", "2. liga interregional"),
    "denmark": ("1st division", "1. division", "2nd division", "2. division", "3rd division", "3. division", "denmark series"),
    "norway": ("1. division", "2. division", "3. division"),
    "sweden": ("superettan", "ettan", "division 2", "division 3"),
    "poland": ("i liga", "ii liga", "iii liga", "iv liga"),
    "czech republic": ("fnl", "national football league", "cfl", "msfl", "divize"),
    "croatia": ("first nl", "druga nl", "second nl", "third nl"),
    "serbia": ("prva liga", "srpska liga"),
    "romania": ("liga ii", "liga iii"),
    "ukraine": ("persha liga", "druha liga"),
    "greece": ("super league 2", "football league", "gamma ethniki"),
    "hungary": ("nb ii", "nb iii"),
    "slovenia": ("2. snl", "3. snl"),
    "slovakia": ("2. liga", "3. liga"),
    "bulgaria": ("second league", "third league"),
    "finland": ("ykkosliiga", "ykkonen", "kakkonen"),
    "iceland": ("1. deild", "2. deild", "3. deild"),
    "ireland": ("first division",),
    "northern ireland": ("championship", "premier intermediate"),
    "wales": ("cymru north", "cymru south"),
    "cyprus": ("2. division", "second division"),
    "israel": ("liga leumit", "liga alef"),
    "colombia": ("primera b",),
    "chile": ("primera b",),
    "uruguay": ("segunda division", "segunda división"),
    "ecuador": ("serie b",),
    "paraguay": ("division intermedia", "intermedia"),
    "peru": ("liga 2",),
    "japan": ("j2 league", "j3 league", "japan football league"),
    "south korea": ("k league 2", "k3 league", "k4 league"),
    "korea republic": ("k league 2", "k3 league", "k4 league"),
    "australia": ("npl", "state league"),
    "saudi arabia": ("division 1", "first division"),
    "qatar": ("second division",),
}


def _country_lower_tier_reason(country: str, name: str) -> Optional[str]:
    for token in COUNTRY_LOWER_TIER_TOKENS.get(country, ()):
        if token in name:
            return token
    return None


def _team_minor_reason(value: Any) -> Optional[str]:
    """Riconosce giovanili, femminili e seconde squadre dal nome squadra.

    Evita controlli fragili come la singola lettera W contenuta nel nome: W
    viene considerata soltanto come suffisso autonomo.
    """
    ...
    for token in (
        "women", "woman", "ladies", "girls", "femminile", "feminile",
        "femenina", "femenino", "feminino", "feminina", "frauen",
        "reserve", "reserves", "academy", "development",
    ):
        if token in words or token in text:
            return f"team_{token}"

    if "jong" in words:
        return "team_jong_reserve"

    # U17/U18/... anche nelle forme U-19 / U 19.
    for age in range(15, 24):
        if f"u{age}" in words or f"u{age}" in compact:
            return f"team_u{age}"
        for idx in range(len(words) - 1):
            if words[idx] == "u" and words[idx + 1] == str(age):
                return f"team_u{age}"

    # Suffissi tipici delle seconde squadre. Non blocca nomi come B 93.
    if words:
        suffix = words[-1]
        if suffix in {"w", "ii", "iii", "b", "c", "2"}:
            return f"team_suffix_{suffix}"
    return None


def fixture_filter_profile(fixture_row: Mapping[str, Any], excluded_countries: Iterable[str] = ()) -> Dict[str, Any]:
    league = fixture_row.get("league", {}) or {}
    league_profile = competition_profile(league)
    if not league_profile.get("allowed"):
        return dict(league_profile)

    excluded = {_country_key(item) for item in excluded_countries}
    if _country_key(league.get("country")) in excluded:
        return {"allowed": False, "score": 0, "tier": "blocked", "reason": "country_excluded"}

    teams = fixture_row.get("teams", {}) or {}
    for side in ("home", "away"):
        reason = _team_minor_reason((teams.get(side, {}) or {}).get("name"))
        if reason:
            return {"allowed": False, "score": 10, "tier": "blocked", "reason": reason}

    result = dict(league_profile)
    result["fixture_level_checked"] = True
    return result


def fixture_allowed(fixture_row: Mapping[str, Any], excluded_countries: Iterable[str] = ()) -> bool:
    return bool(fixture_filter_profile(fixture_row, excluded_countries).get("allowed"))


def output_fixture_allowed(item: Mapping[str, Any], excluded_countries: Iterable[str] = ()) -> bool:
    fixture = item.get("fixture", {}) or {}
    pseudo = {
        "league": {"name": fixture.get("league"), "country": fixture.get("country")},
        "teams": {
            "home": {"name": fixture.get("home")},
            "away": {"name": fixture.get("away")},
        },
    }
    return fixture_allowed(pseudo, excluded_countries)
...
def competition_profile(league: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # Eccezione mirata ArabSniper: la Serie C italiana resta un campionato
    # selezionabile. Non riapre le altre terze divisioni né la Serie D.
    # Il punteggio 68 la mantiene sotto Serie A/B, ma sopra il gate qualità
    # minimo (58) quando quote, statistiche e affidabilità confermano il segnale.
    is_italian_serie_c = country == "italy" and "serie c" in name
    if is_italian_serie_c:
        return {
            "allowed": True,
            "score": 68,
            "tier": "italy_serie_c",
            "reason": "trusted_italian_serie_c",
        }

    country_lower_token = _country_lower_tier_reason(country, name)
    if country_lower_token:
        return {
            "allowed": False, "score": 25, "tier": "blocked",
            "reason": f"country_lower_tier:{country_lower_token}",
        }
...
def scan_dates(client: APIClient, target_dates: Sequence[str], excluded_countries: Sequence[str]) -> Dict[str, Any]:
    ...
    output.setdefault("method", {})["competition_filter"] = "whitelist qualitativa + filtro fixture: Serie C italiana ammessa; U15-U23, femminili, riserve/II/B/Jong/academy e livelli inferiori esteri esclusi prima delle chiamate pesanti"
...
HARD_MINOR_TOKENS = HARD_MINOR_TOKENS + (
    ...
    "frauen", "wk league", "liga f women", "mls next pro",
    "oberliga", "regionalliga", "highland league", "lowland league",
    "tweede divisie", "derde divisie", "campeonato de portugal",
)
...
# =========================================================
# ENHANCEMENT V1.1.5 — 1X2 -> GOAL REGIME + VALUE SHORTLIST
# Mantiene lo schema web V1.1.x e aggiunge soltanto campi.
# =========================================================
VERSION = "1.1.7"


def _profile_metric(profile: Mapping[str, Any], key: str) -> float:
    return safe_float((profile.get("metrics", {}) or {}).get(key), 0.0)


def _profile_aux(profile: Mapping[str, Any], section: str, key: str) -> float:
    return safe_float((profile.get(section, {}) or {}).get(key), 0.0)


def _market_side_snapshot(market: Mapping[str, Any], side: str) -> Dict[str, float]:
    selection = (market.get("selections", {}) or {}).get(side, {}) or {}
    return {
        "open_odd": _selection_odd(market, "open", side),
        "current_odd": _selection_odd(market, "current", side),
        "open_fair": _selection_fair(market, "open", side),
        "current_fair": _selection_fair(market, "current", side),
        "delta_fair_pp": safe_float(selection.get("delta_fair_pp"), 0.0),
        "oci": safe_float((selection.get("oci_as", {}) or {}).get("value"), 0.0),
        "consensus": safe_float((selection.get("bookmaker_consensus", {}) or {}).get("ratio"), 0.0),
        "margin_delta_pp": safe_float(selection.get("margin_load_delta_pp"), 0.0),
    }
...
def analyze_favorite_market_pattern(output: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    Non assume che un movimento sia una 'trappola'. Misura quattro famiglie di
    segnali e richiede sempre conferma statistica prima di correggere le
    probabilita': inversione 1/2, steam della favorita nella fascia medio-bassa,
    quadrato book sullo sfavorito e redistribuzione del margine.
    """
    book_markets = (output.get("book", {}) or {}).get("markets", {}) or {}
    one_x_two = book_markets.get(MARKET_1X2, {}) or {}
    stats = output.get("statistics", {}) or {}
    ...
    home_side = _market_side_snapshot(one_x_two, "1")
    away_side = _market_side_snapshot(one_x_two, "2")
    sides = {"1": home_side, "2": away_side}
    if max(home_side["current_fair"], away_side["current_fair"]) <= 0:
        return {"available": False, "score": 0.0, "classification": "unavailable"}

    opening_favorite = (
        max(("1", "2"), key=lambda side: sides[side]["open_fair"])
        if max(home_side["open_fair"], away_side["open_fair"]) > 0 else None
    )
    current_favorite = max(("1", "2"), key=lambda side: sides[side]["current_fair"])
    current_underdog = "2" if current_favorite == "1" else "1"
    favorite = sides[current_favorite]
    underdog = sides[current_underdog]
    favorite_profile = home if current_favorite == "1" else away
    underdog_profile = away if current_favorite == "1" else home

    cross_flip = bool(opening_favorite and opening_favorite != current_favorite)
    cross_strength_pp = min(abs(home_side["delta_fair_pp"]), abs(away_side["delta_fair_pp"]))
    favorite_shortening = (
        favorite["open_odd"] - favorite["current_odd"]
        if min(favorite["open_odd"], favorite["current_odd"]) > 1 else 0.0
    )
    favorite_drift = -favorite_shortening
    underdog_shortening = (
        underdog["open_odd"] - underdog["current_odd"]
        if min(underdog["open_odd"], underdog["current_odd"]) > 1 else 0.0
    )
    medium_favorite_band = any(
        1.60 <= odd <= 1.90 for odd in (favorite["open_odd"], favorite["current_odd"])
        if odd > 1
    )
    focal_170_band = any(
        1.66 <= odd <= 1.80 for odd in (favorite["open_odd"], favorite["current_odd"])
        if odd > 1
    )
    favorite_steam = favorite_shortening >= 0.04 and favorite["delta_fair_pp"] >= 0.70
    favorite_steam_band = medium_favorite_band and favorite_steam

    book_pack = one_x_two.get("book", {}) or {}
    book_pick = str(book_pack.get("pick") or "")
    book_square_underdog = book_pick == current_underdog
    book_square_favorite = book_pick == current_favorite
    protection_pick = str(book_pack.get("protection_pick") or "")
    margin_shift_favorite_vs_underdog = favorite["margin_delta_pp"] - underdog["margin_delta_pp"]

    favorite_concedes_ht = _profile_metric(favorite_profile, "conceded_ht_rate")
    favorite_scores_ht = _profile_metric(favorite_profile, "scored_ht_rate")
    underdog_scores_ht = _profile_metric(underdog_profile, "scored_ht_rate")
    underdog_concedes_ht = _profile_metric(underdog_profile, "conceded_ht_rate")
    home_goal_ht = _profile_metric(home, "goal_ht_rate")
    away_goal_ht = _profile_metric(away, "goal_ht_rate")
    home_o15_ht = _profile_metric(home, "over15ht_rate")
    away_o15_ht = _profile_metric(away, "over15ht_rate")
    home_gg_ht = _profile_metric(home, "gg_ht_rate")
    away_gg_ht = _profile_metric(away, "gg_ht_rate")
    pair_goal_ht = mean((home_goal_ht, away_goal_ht))
    pair_o15_ht = mean((home_o15_ht, away_o15_ht))
    pair_gg_ht = mean((home_gg_ht, away_gg_ht))
    pair_over25 = mean((_profile_metric(home, "over25_rate"), _profile_metric(away, "over25_rate"), safe_float(league.get("over25_rate"), 0.50)))

    underdog_xg_for = _profile_metric(underdog_profile, "xg_for_ft")
    favorite_xg_against = _profile_metric(favorite_profile, "xg_against_ft")
    underdog_sot_for = _profile_aux(underdog_profile, "shot_quality", "shots_on_target_for")
    favorite_sot_against = _profile_aux(favorite_profile, "shot_quality", "shots_on_target_against")
    threat_aux = 0.0
    if underdog_xg_for > 0 and favorite_xg_against > 0:
        threat_aux += clamp((mean((underdog_xg_for, favorite_xg_against)) - 0.85) / 0.70, 0.0, 1.0) * 6.0
    if underdog_sot_for > 0 and favorite_sot_against > 0:
        threat_aux += clamp((mean((underdog_sot_for, favorite_sot_against)) - 2.6) / 2.4, 0.0, 1.0) * 5.0

    market_score = 0.0
    market_score += 30.0 if cross_flip else 0.0
    market_score += clamp(cross_strength_pp / 3.0, 0.0, 1.0) * 10.0 if cross_flip else 0.0
    market_score += 19.0 if favorite_steam_band else 0.0
    market_score += 5.0 if focal_170_band and favorite_steam else 0.0
    market_score += 18.0 if book_square_underdog else 0.0
    market_score += clamp(max(underdog["oci"], 0.0) / 65.0, 0.0, 1.0) * 8.0
    market_score += clamp(underdog["consensus"], 0.0, 1.0) * 5.0 if book_square_underdog or underdog["oci"] > 0 else 0.0
    if protection_pick == current_favorite and favorite_steam_band:
        market_score += 5.0
    if margin_shift_favorite_vs_underdog > 0.02 and favorite_steam_band:
        market_score += clamp(margin_shift_favorite_vs_underdog / 0.18, 0.0, 1.0) * 5.0
    market_score = clamp(market_score, 0.0, 100.0)

    early_stats_score = (
        clamp((pair_goal_ht - 0.52) / 0.30, 0.0, 1.0) * 25.0 +
        clamp((favorite_scores_ht - 0.30) / 0.40, 0.0, 1.0) * 15.0 +
        clamp((underdog_scores_ht - 0.20) / 0.40, 0.0, 1.0) * 22.0 +
        clamp((favorite_concedes_ht - 0.22) / 0.38, 0.0, 1.0) * 22.0 +
        clamp((underdog_concedes_ht - 0.25) / 0.40, 0.0, 1.0) * 6.0 +
        threat_aux
    )
    early_stats_score = clamp(early_stats_score, 0.0, 100.0)
    two_goal_stats_score = clamp(
        clamp((pair_o15_ht - 0.24) / 0.30, 0.0, 1.0) * 46.0 +
        clamp((pair_gg_ht - 0.08) / 0.22, 0.0, 1.0) * 24.0 +
        clamp((min(favorite_scores_ht, underdog_scores_ht) - 0.22) / 0.30, 0.0, 1.0) * 30.0,
        0.0, 100.0,
    )
    ft_goal_score = clamp(
        clamp((pair_over25 - 0.43) / 0.27, 0.0, 1.0) * 55.0 +
        (early_stats_score * 0.20) + (two_goal_stats_score * 0.25),
        0.0, 100.0,
    )

    early_goal_confirmation = pair_goal_ht >= 0.57 or early_stats_score >= 48.0
    underdog_threat_confirmation = favorite_concedes_ht >= 0.27 and underdog_scores_ht >= 0.24
    two_way_confirmation = (
        min(favorite_scores_ht, underdog_scores_ht) >= 0.24 or
        pair_gg_ht >= 0.11
    )
    over15_confirmation = pair_o15_ht >= 0.27 or two_goal_stats_score >= 46.0
    over25_confirmation = pair_over25 >= 0.46 or ft_goal_score >= 50.0
    statistical_confirmation = early_goal_confirmation and (
        underdog_threat_confirmation or over15_confirmation or two_way_confirmation
    )

    combined_score = clamp(market_score * 0.58 + early_stats_score * 0.24 + two_goal_stats_score * 0.18, 0.0, 100.0)
    if cross_flip and favorite_steam_band and book_square_underdog and statistical_confirmation:
        classification = "multi_signal_goal_pressure"
    elif cross_flip and statistical_confirmation:
        classification = "odds_cross_reversal"
    elif favorite_steam_band and underdog_threat_confirmation:
        classification = "favorite_steam_vulnerable"
    elif book_square_underdog and underdog_threat_confirmation:
        classification = "underdog_book_square"
    elif cross_flip:
        classification = "odds_inversion_only"
    elif favorite_steam_band:
        classification = "favorite_steam_only"
    else:
        classification = "neutral"

    return {
        "available": True,
        "score": round3(combined_score),
        "market_score": round3(market_score),
        "early_stats_score": round3(early_stats_score),
        "two_goal_stats_score": round3(two_goal_stats_score),
        "ft_goal_score": round3(ft_goal_score),
        "classification": classification,
        "experimental": True,
        "opening_favorite": opening_favorite,
        "current_favorite": current_favorite,
        "favorite_changed": cross_flip,
        "cross_strength_pp": round3(cross_strength_pp),
        "favorite_team": (output.get("fixture", {}) or {}).get("home") if current_favorite == "1" else (output.get("fixture", {}) or {}).get("away"),
        "underdog_team": (output.get("fixture", {}) or {}).get("away") if current_favorite == "1" else (output.get("fixture", {}) or {}).get("home"),
        "favorite_open_odd": round3(favorite["open_odd"]),
        "favorite_current_odd": round3(favorite["current_odd"]),
        "underdog_open_odd": round3(underdog["open_odd"]),
        "underdog_current_odd": round3(underdog["current_odd"]),
        "favorite_shortening_odd": round3(favorite_shortening),
        "favorite_drift_odd": round3(favorite_drift),
        "underdog_steam_odd": round3(underdog_shortening),
        "medium_favorite_band": medium_favorite_band,
        "focal_170_180_band": focal_170_band,
        "favorite_steam": favorite_steam,
        "favorite_steam_band": favorite_steam_band,
        "favorite_fair_delta_pp": round3(favorite["delta_fair_pp"]),
        "underdog_fair_delta_pp": round3(underdog["delta_fair_pp"]),
        "favorite_oci": round3(favorite["oci"]),
        "underdog_oci": round3(underdog["oci"]),
        "underdog_book_consensus": round4(underdog["consensus"]),
        "book_pick_1x2": book_pick or None,
        "book_square_underdog": book_square_underdog,
        "book_square_favorite": book_square_favorite,
        "protection_pick": protection_pick or None,
        "favorite_margin_delta_pp": round3(favorite["margin_delta_pp"]),
        "underdog_margin_delta_pp": round3(underdog["margin_delta_pp"]),
        "margin_shift_favorite_vs_underdog_pp": round3(margin_shift_favorite_vs_underdog),
        "favorite_conceded_ht_rate_pct": round3(favorite_concedes_ht * 100.0),
        "underdog_scored_ht_rate_pct": round3(underdog_scores_ht * 100.0),
        "favorite_scored_ht_rate_pct": round3(favorite_scores_ht * 100.0),
        "underdog_conceded_ht_rate_pct": round3(underdog_concedes_ht * 100.0),
        "pair_goal_ht_rate_pct": round3(pair_goal_ht * 100.0),
        "pair_over15_ht_rate_pct": round3(pair_o15_ht * 100.0),
        "pair_gg_ht_rate_pct": round3(pair_gg_ht * 100.0),
        "pair_over25_ft_rate_pct": round3(pair_over25 * 100.0),
        "underdog_xg_for": round3(underdog_xg_for),
        "favorite_xg_against": round3(favorite_xg_against),
        "underdog_sot_for": round3(underdog_sot_for),
        "favorite_sot_against": round3(favorite_sot_against),
        "early_goal_confirmation": early_goal_confirmation,
        "underdog_threat_confirmation": underdog_threat_confirmation,
        "two_way_confirmation": two_way_confirmation,
        "over15_confirmation": over15_confirmation,
        "over25_confirmation": over25_confirmation,
        "statistical_confirmation": statistical_confirmation,
        "interpretation": (
            "Il movimento 1X2 e' una feature, non una prova di trappola. Il correttivo goal si attiva "
            "solo quando inversione/steam/quadrato book trovano conferma nelle frequenze PT, capacita' di "
            "segnare/subire presto e, quando disponibili, xG e tiri nello specchio."
        ),
    }


def _set_binary_positive(raw_probabilities: MutableMapping[str, Any], market_key: str, positive: str, value: float) -> None:
    negative = "no" if positive == "yes" else "under"
    raw_probabilities[market_key] = {
        positive: round4(clamp(value, 0.01, 0.99)),
        negative: round4(1.0 - clamp(value, 0.01, 0.99)),
    }


def _cohere_first_half_probabilities(raw_probabilities: MutableMapping[str, Any]) -> None:
    """Impedisce incoerenze logiche: GG PT <= O1.5 PT <= Goal PT."""
    p05 = safe_float((raw_probabilities.get(MARKET_O05HT, {}) or {}).get("over"), 0.0)
    p15 = safe_float((raw_probabilities.get(MARKET_O15HT, {}) or {}).get("over"), 0.0)
    pgg = safe_float((raw_probabilities.get(MARKET_GGHT, {}) or {}).get("yes"), 0.0)
    if p05 > 0 and p15 > 0:
        p15 = min(p15, max(p05 - 0.01, 0.01))
        _set_binary_positive(raw_probabilities, MARKET_O15HT, "over", p15)
    if p15 > 0 and pgg > 0:
        pgg = min(pgg, p15)
        _set_binary_positive(raw_probabilities, MARKET_GGHT, "yes", pgg)


def _apply_favorite_trap_adjustment(raw_probabilities: MutableMapping[str, Any], pattern: Mapping[str, Any]) -> Dict[str, float]:
    """Correttivo piccolo e auditabile, esteso anche all'Over 2.5 FT."""
    adjustments = {MARKET_O05HT: 0.0, MARKET_O15HT: 0.0, MARKET_GGHT: 0.0, MARKET_O25: 0.0}
    if not pattern.get("available"):
        return adjustments

    market_score = safe_float(pattern.get("market_score"), 0.0)
    if market_score < 25.0:
        return adjustments
    intensity = clamp((safe_float(pattern.get("score"), 0.0) - 32.0) / 45.0, 0.0, 1.0)
    if intensity <= 0:
        return adjustments

    cross = bool(pattern.get("favorite_changed"))
    steam_band = bool(pattern.get("favorite_steam_band"))
    square_underdog = bool(pattern.get("book_square_underdog"))
    early_ok = bool(pattern.get("early_goal_confirmation"))
    threat_ok = bool(pattern.get("underdog_threat_confirmation"))
    two_way_ok = bool(pattern.get("two_way_confirmation"))
    o15_ok = bool(pattern.get("over15_confirmation"))
    o25_ok = bool(pattern.get("over25_confirmation"))

    if early_ok:
        base = (0.008 if cross else 0.0) + (0.006 if steam_band else 0.0) + (0.004 if square_underdog else 0.0)
        adjustments[MARKET_O05HT] = min(0.016, base * intensity)
    if early_ok and (o15_ok or threat_ok):
        base = (0.018 if cross else 0.0) + (0.010 if steam_band else 0.0) + (0.009 if square_underdog else 0.0)
        adjustments[MARKET_O15HT] = min(0.032, base * intensity)
    if threat_ok and two_way_ok:
        base = (0.022 if cross else 0.0) + (0.014 if steam_band else 0.0) + (0.012 if square_underdog else 0.0)
        adjustments[MARKET_GGHT] = min(0.038, base * intensity)
    if o25_ok and (o15_ok or threat_ok):
        base = (0.012 if cross else 0.0) + (0.008 if steam_band else 0.0) + (0.006 if square_underdog else 0.0)
        adjustments[MARKET_O25] = min(0.025, base * intensity)

    for market_key, delta in adjustments.items():
        if delta <= 0:
            continue
        positive = "yes" if market_key == MARKET_GGHT else "over"
        current = safe_float((raw_probabilities.get(market_key, {}) or {}).get(positive), 0.0)
        if current <= 0:
            continue
        _set_binary_positive(raw_probabilities, market_key, positive, current + delta)
    _cohere_first_half_probabilities(raw_probabilities)
    return {key: round4(value) for key, value in adjustments.items()}


_recommendation_gate_v114 = recommendation_gate
...
def recommendation_gate(
    ...
    gate = _recommendation_gate_v114(
        market_key, pick, raw_probabilities, calibrated_probabilities,
        market_analysis, validation, reliability_pct, meta, relation, empirical_layer,
    )
    if not pick:
        return gate
    odd = safe_float(((market_analysis or {}).get("current", {}) or {}).get("odds", {}).get(pick), 0.0)
    ev = safe_float(gate.get("expected_value_pct"), -100.0)
    analytical_score = safe_float(gate.get("score"), 0.0)
    adjustment = 0.0
    if odd <= 1.0:
        price_profile = "missing"
    elif odd < 1.60:
        price_profile = "low_price"
        if market_key in {MARKET_O05HT, MARKET_O25}:
            adjustment -= 5.0
    elif odd < 1.75:
        price_profile = "below_preferred"
        adjustment -= 1.5
    elif odd <= 3.50:
        price_profile = "preferred_value_band"
        if ev >= 1.0:
            adjustment += 3.0
    elif odd <= 6.00:
        price_profile = "high_return_band"
        if ev >= 4.0 and reliability_pct >= 58:
            adjustment += 2.0
        elif reliability_pct < 58:
            adjustment -= 2.0
    else:
        price_profile = "very_high_price"
        adjustment -= 4.0

    adjusted_score = clamp(analytical_score + adjustment, 0.0, 100.0)
    gate["analytical_score_before_price_preference"] = round3(analytical_score)
    gate["price_preference_adjustment"] = round3(adjustment)
    gate["price_profile"] = price_profile
    gate["score"] = round3(adjusted_score)
    # La preferenza di quota modifica il ranking, non rende valida una giocata non valida.
    if gate.get("eligible"):
        gate["grade"] = "A" if adjusted_score >= 79 else "B" if adjusted_score >= 69 else "C"
    return gate


_preserve_existing_match_v114 = _preserve_existing_match


def _preserve_existing_match(item: Mapping[str, Any], status_row: Optional[Mapping[str, Any]], reason: str) -> Dict[str, Any]:
    preserved = _preserve_existing_match_v114(item, status_row, reason)
    result = ((preserved.get("fixture", {}) or {}).get("result") or {})
    if not result:
        return preserved
    pattern = ((preserved.get("model", {}) or {}).get("favorite_trap_pattern") or {})
    if not pattern:
        return preserved
    ht_total = safe_int(result.get("home_ht")) + safe_int(result.get("away_ht"))
    ft_total = safe_int(result.get("home_ft")) + safe_int(result.get("away_ft"))
    audit = preserved.setdefault("prediction_audit", {})
    audit["market_pattern"] = json.loads(json.dumps(pattern))
    audit["pattern_outcomes"] = {
        "goal_ht": ht_total >= 1,
        "over15_ht": ht_total >= 2,
        "gg_ht": safe_int(result.get("home_ht")) > 0 and safe_int(result.get("away_ht")) > 0,
        "over25_ft": ft_total >= 3,
    }
    return preserved


_update_prediction_audit_v114 = update_prediction_audit


def _pattern_audit_summary(records: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    groups: Dict[str, Any] = {}
    for record in records:
        pattern = record.get("market_pattern") or {}
        outcomes = record.get("pattern_outcomes") or {}
        if not pattern or not outcomes:
            continue
        key = str(pattern.get("classification") or "unknown")
        bucket = groups.setdefault(key, {
            "settled": 0, "goal_ht": 0, "over15_ht": 0, "gg_ht": 0, "over25_ft": 0,
            "avg_pattern_score": 0.0, "_score_sum": 0.0,
        })
        bucket["settled"] += 1
        bucket["_score_sum"] += safe_float(pattern.get("score"), 0.0)
        for event in ("goal_ht", "over15_ht", "gg_ht", "over25_ft"):
            if bool(outcomes.get(event)):
                bucket[event] += 1
    for bucket in groups.values():
        settled = max(safe_int(bucket.get("settled")), 1)
        bucket["avg_pattern_score"] = round3(safe_float(bucket.pop("_score_sum", 0.0)) / settled)
        for event in ("goal_ht", "over15_ht", "gg_ht", "over25_ft"):
            bucket[f"{event}_rate_pct"] = round3(safe_int(bucket.get(event)) / settled * 100.0)
    return groups


def update_prediction_audit(output: Mapping[str, Any]) -> Dict[str, Any]:
    store = _update_prediction_audit_v114(output)
    fixtures = store.setdefault("fixtures", {})
    for day in output.get("days", []) or []:
        for match in day.get("matches", []) or []:
            fixture = match.get("fixture", {}) or {}
            fixture_id = str(fixture.get("fixture_id") or "")
            audit = match.get("prediction_audit") or {}
            pattern = audit.get("market_pattern") or {}
            outcomes = audit.get("pattern_outcomes") or {}
            result = audit.get("result") or fixture.get("result") or {}
            if not fixture_id or not pattern or not outcomes or not result:
                continue
            record = fixtures.setdefault(fixture_id, {
                "fixture_id": fixture_id,
                "date": fixture.get("date"), "time": fixture.get("time"),
                "match": fixture.get("match"), "league": fixture.get("league"),
                "country": fixture.get("country"),
                "competition_quality_score": fixture.get("competition_quality_score"),
                "competition_tier": fixture.get("competition_tier"),
                "result": result, "primary": audit.get("primary"),
                "settled_at": (result or {}).get("settled_at", iso_now()),
            })
            record["market_pattern"] = json.loads(json.dumps(pattern))
            record["pattern_outcomes"] = dict(outcomes)
    base_summary = _audit_summary(fixtures.values())
    base_summary["by_market_pattern"] = _pattern_audit_summary(fixtures.values())
    store["version"] = VERSION
    store["updated_at"] = iso_now()
    store["summary"] = base_summary
    atomic_write_json(PREDICTION_AUDIT_FILE, store)
    return store


def _focused_day_shortlist(day: Mapping[str, Any]) -> Dict[str, Any]:
    rows: List[Dict[str, Any]] = []
    for match in day.get("matches", []) or []:
        fixture = match.get("fixture", {}) or {}
        ...
        for choice in ranked:
            odd = safe_float(choice.get("odd"), 0.0)
            gate = choice.get("gate", {}) or {}
            if not gate.get("eligible"):
                continue
            if not (1.70 <= odd <= 6.00):
                continue
            if safe_float(choice.get("recommendation_score"), 0.0) < 66.0:
                continue
            best = choice
            break
        if not best:
        ...
        rows.append({
            ...
            "odd": round3(best.get("odd")),
            "probability_pct": round3(best.get("probability_pct")),
            "expected_value_pct": round3(best.get("expected_value_pct")),
            "grade": best.get("grade"),
            "score": round3(best.get("recommendation_score")),
            "ranking_score": round3(best.get("ranking_score")),
            "price_profile": gate.get("price_profile"),
            "relation": best.get("relation"),
        })
    rows.sort(key=lambda row: (safe_float(row.get("score")), safe_float(row.get("expected_value_pct"))), reverse=True)
    top = rows[:3]

    def combo(items: Sequence[Mapping[str, Any]]) -> Optional[Dict[str, Any]]:
        if len(items) < 2:
            return None
        odd_product = 1.0
        probability_product = 1.0
        for item in items:
            odd_product *= max(safe_float(item.get("odd"), 1.0), 1.0)
            probability_product *= clamp(safe_float(item.get("probability_pct"), 0.0) / 100.0, 0.0, 1.0)
        return {
            "legs": len(items),
            "combined_odd": round3(odd_product),
            "estimated_hit_probability_pct": round3(probability_product * 100.0),
            "estimated_combined_ev_pct": round3((probability_product * odd_product - 1.0) * 100.0),
            "fixture_ids": [item.get("fixture_id") for item in items],
            "assumption": "probabilita moltiplicate come approssimazione; gli eventi reali possono non essere perfettamente indipendenti",
        }

    return {
        "policy": "massimo 3 match, uno per partita, quota 1.70-6.00, solo segnali gia' eleggibili",
        "candidates": top,
        "best_2": combo(top[:2]) if len(top) >= 2 else None,
        "best_3": combo(top[:3]) if len(top) >= 3 else None,
        "note": "La shortlist non aumenta artificialmente la probabilita': riduce soltanto il numero di variabili rispetto a multiple lunghe.",
    }


_scan_dates_v114 = scan_dates


def scan_dates(client: APIClient, target_dates: Sequence[str], excluded_countries: Sequence[str]) -> Dict[str, Any]:
    output = _scan_dates_v114(client, target_dates, excluded_countries)
    output["version"] = VERSION
    for day in output.get("days", []) or []:
        day["focused_shortlist"] = _focused_day_shortlist(day)
    output.setdefault("method", {})["market_goal_regime"] = (
        "cross/inversione 1X2 + steam favorita 1.60-1.90 + quadrato book sullo sfavorito + OCI/consenso/margine; "
        "nessun boost senza conferma statistica PT"
    )
    output["method"]["ft_bridge"] = (
        "Over 2.5 FT puo' ricevere una correzione massima di 2.5 punti percentuali quando il regime 1X2 e il profilo PT/FT concordano"
    )
    output["method"]["probability_coherence"] = "vincolo logico GG PT <= Over 1.5 PT <= Goal PT"
    output["method"]["focused_shortlist"] = (
        "ranking separato per 2-3 match con quota 1.70-6.00; la preferenza di prezzo modifica il ranking ma non rende valido un segnale debole"
    )
    output["method"]["pattern_audit"] = (
        "prediction audit esteso a goal PT, over 1.5 PT, GG PT e over 2.5 FT per ogni classificazione del regime quote"
    )
    atomic_write_json(OUTPUT_FILE, output)
    return output
