...
def build_signal_package(fid, mk, s_h, s_a):
    ...
    # -------------------------------------------------
    if match_profile == "early_pressure":
        internal_labels.append("EARLY")
    elif match_profile == "open_match":
        internal_labels.append("OPEN")
    elif match_profile == "favorite_pressure":
        internal_labels.append("FAV_PRESS")
    elif match_profile == "asymmetric":
        internal_labels.append("ASYM")
    elif match_profile == "control_risk":
        internal_labels.append("CONTROL")
    elif match_profile == "low_event":
        internal_labels.append("LOW_EVT")
    else:
        internal_labels.append("NEUTRAL")

    if structure_grade == "high":
        internal_labels.append("STRONG")
    elif structure_grade == "medium":
        internal_labels.append("MEDIUM")
    else:
        internal_labels.append("FRAGILE")

    if coherence_level == "high":
        internal_labels.append("COHERENT")
    elif coherence_level == "medium":
        internal_labels.append("PARTIAL")
    else:
        internal_labels.append("NOISY")

    if dislocation_level == "high":
        internal_labels.append("VALUE_GAP")
    elif dislocation_level == "medium":
        internal_labels.append("LIGHT_GAP")
    ...
    # -------------------------------------------------
    # LAYER 4 - GOLD
    # GOLD = partita forte e leggibile, non solo potente
    # -------------------------------------------------
    gold_has_over = ("⚽ OVER" in tags)

    gold_gate_structure = (
        structure_score >= 1.55
        and structure_grade in ("high", "medium")
    ...
    gold_gate_market = (
        is_gold_zone
        and coherence_score >= 1.95
        and 1.52 <= safe_float(mk.get("o25"), 0.0) <= 2.20
        and 1.18 <= safe_float(mk.get("o05ht"), 0.0) <= 1.40
        and value_left != "low"
    )

    gold_gate_readability = (
        market_profile in ("full_open", "balanced_open", "favorite_pressure_open", "favorite_control")
        and not has_warning(market_pack, "ft_market_ahead_of_structure")
        and not has_warning(market_pack, "o25_too_low_for_one_sided_ft")
        and not has_warning(market_pack, "favorite_ultra_but_ft_structure_weak")
    )
    ...
    gold_gate_extra = (
        drop_confirmed
        or over_score >= 4.70
        or dislocation_score >= 0.55
        or (combined_ft_clean >= 1.78 and combined_ht_scored_clean >= 0.78)
    )
    ...
    # -------------------------------------------------
    # LAYER 5 - PROBE
    # Più pulite del passato: non devono sporcare GOLD/BOOST
    # -------------------------------------------------
    probe_o_ok = (
        "⚽ OVER" not in tags
        ...
        and safe_float(s_h.get("ft_2plus_rate", 0.0), 0.0) >= 0.50
        and safe_float(s_a.get("ft_2plus_rate", 0.0), 0.0) >= 0.50
        and 1.55 <= safe_float(mk.get("o25"), 0.0) <= 2.32
        and safe_float(mk.get("o05ht"), 0.0) <= 1.44
        and coherence_score >= 1.25
        and over_score >= 3.55
        and value_left != "low"
        and not has_warning(market_pack, "ft_market_ahead_of_structure")
    ...
    probe_g_ok = (
        ...
        and 1.52 <= safe_float(mk.get("o25"), 0.0) <= 2.35
        and pt_score >= 3.65
        and over_score >= 3.65
