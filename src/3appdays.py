...
# ==========================================
# ARAB SNIPER HYBRID CLASSIC - V25 SHELL / V24 BRAIN
# BLOCCO 1
# - import / config
# - helper base
# - github sync
...
st.set_page_config(page_title="ARAB SNIPER HYBRID CLASSIC", layout="wide")
...
def parse_rome_ts(ts_value):
    """
    Parsing robusto timestamp Europe/Rome per la history quote.
    """
    try:
        if not ts_value:
            return None
        raw = str(ts_value).strip().replace("T", " ")[:19]
...
def build_quote_history_point(mk, ts=None):
    ts = ts or now_rome().strftime("%Y-%m-%d %H:%M:%S")
    return {
        "ts": ts,
        "hhmm": str(ts)[11:16] if len(str(ts)) >= 16 else now_rome().strftime("%H:%M"),
        "q1": round3(safe_float(mk.get("q1", mk.get("Q1_CURR", 0.0)), 0.0)),
        "qx": round3(safe_float(mk.get("qx", mk.get("QX_CURR", 0.0)), 0.0)),
        "q2": round3(safe_float(mk.get("q2", mk.get("Q2_CURR", 0.0)), 0.0)),
        "o25": round3(safe_float(mk.get("o25", mk.get("O25_CURR", 0.0)), 0.0)),
        "o05ht": round3(safe_float(mk.get("o05ht", mk.get("O05HT_CURR", 0.0)), 0.0)),
        "o15ht": round3(safe_float(mk.get("o15ht", mk.get("O15HT_CURR", 0.0)), 0.0)),
    }


def append_quote_history(history, mk, max_points=12):
    """
    Aggiunge il punto corrente senza duplicare lo stesso slot orario.
    Mantiene solo gli ultimi max_points per non gonfiare i JSON.
    """
    ...
    for h in history:
        ...
        if not ts or ts in seen:
            continue
        seen.add(ts)
        clean.append(build_quote_history_point(h, ts=ts))

    now_ts = now_rome().strftime("%Y-%m-%d %H:%M:%S")
    current = build_quote_history_point(mk, ts=now_ts)

    # se l'ultimo punto è nello stesso minuto, aggiorna invece di duplicare
    if clean and str(clean[-1].get("ts", ""))[:16] == now_ts[:16]:
        clean[-1] = current
    else:
        clean.append(current)

    clean = sorted(clean, key=lambda x: str(x.get("ts", "")))
    return clean[-max_points:]


def _quote_velocity(prev_point, curr_point, key):
    prev_dt = parse_rome_ts(prev_point.get("ts"))
    curr_dt = parse_rome_ts(curr_point.get("ts"))
    ...
    old_q = safe_float(prev_point.get(key), 0.0)
    new_q = safe_float(curr_point.get(key), 0.0)
    if old_q <= 0 or new_q <= 0:
        return 0.0

    # positivo = quota in discesa
    return round3((old_q - new_q) / hours)


def _classify_velocity(velocity, acceleration):
    velocity = safe_float(velocity, 0.0)
    acceleration = safe_float(acceleration, 0.0)

    if velocity <= 0.005:
        return "flat"
    if acceleration >= 0.020 and velocity >= 0.030:
        return "accelerating"
    if velocity >= 0.050:
        return "fast"
    return "linear"
...
def analyze_quote_history(history):
    """
    Legge la timeline quote e separa:
    - movimento lineare
    - movimento veloce
    - accelerazione recente

    Per ora è diagnostico: NON cambia ancora i tag.
    """
    history = [h for h in (history or []) if isinstance(h, dict)]
    history = sorted(history, key=lambda x: str(x.get("ts", "")))

    empty = {
        "history_count": len(history),
        "o25_velocity": 0.0, "o25_prev_velocity": 0.0, "o25_acceleration": 0.0, "o25_movement_type": "none",
        "o05ht_velocity": 0.0, "o05ht_prev_velocity": 0.0, "o05ht_acceleration": 0.0, "o05ht_movement_type": "none",
        "o15ht_velocity": 0.0, "o15ht_prev_velocity": 0.0, "o15ht_acceleration": 0.0, "o15ht_movement_type": "none",
        "fav_velocity": 0.0, "fav_prev_velocity": 0.0, "fav_acceleration": 0.0, "fav_movement_type": "none",
        "market_acceleration_label": "NO_HISTORY",
    }
    ...
    prev = history[-2]
    curr = history[-1]
    prevprev = history[-3] if len(history) >= 3 else None

    def pair(key):
        v_now = _quote_velocity(prev, curr, key)
        v_prev = _quote_velocity(prevprev, prev, key) if prevprev else 0.0
        accel = round3(v_now - v_prev)
        return v_now, v_prev, accel, _classify_velocity(v_now, accel)

    o25_v, o25_pv, o25_a, o25_t = pair("o25")
    o05_v, o05_pv, o05_a, o05_t = pair("o05ht")
    o15_v, o15_pv, o15_a, o15_t = pair("o15ht")

    # Favorita corrente: prendiamo la min tra 1 e 2 per leggere la pressione 1X2
    fav_hist = []
    for h in history:
        q1 = safe_float(h.get("q1"), 0.0)
        q2 = safe_float(h.get("q2"), 0.0)
        fav_q = min(q1, q2) if q1 > 0 and q2 > 0 else 0.0
        tmp = dict(h)
        tmp["fav"] = fav_q
        fav_hist.append(tmp)

    fav_v = _quote_velocity(fav_hist[-2], fav_hist[-1], "fav")
    fav_pv = _quote_velocity(fav_hist[-3], fav_hist[-2], "fav") if len(fav_hist) >= 3 else 0.0
    fav_a = round3(fav_v - fav_pv)
    fav_t = _classify_velocity(fav_v, fav_a)
    ...
    if accelerating_keys:
        label = "ACCEL_" + "+".join(accelerating_keys)
    elif any(t == "fast" for t in [o25_t, o05_t, o15_t, fav_t]):
        label = "FAST_MOVE"
    elif any(t == "linear" for t in [o25_t, o05_t, o15_t, fav_t]):
        label = "LINEAR_MOVE"
    else:
        label = "FLAT"
    ...
    return {
        "history_count": len(history),
        "o25_velocity": o25_v, "o25_prev_velocity": o25_pv, "o25_acceleration": o25_a, "o25_movement_type": o25_t,
        "o05ht_velocity": o05_v, "o05ht_prev_velocity": o05_pv, "o05ht_acceleration": o05_a, "o05ht_movement_type": o05_t,
        "o15ht_velocity": o15_v, "o15ht_prev_velocity": o15_pv, "o15ht_acceleration": o15_a, "o15ht_movement_type": o15_t,
        "fav_velocity": fav_v, "fav_prev_velocity": fav_pv, "fav_acceleration": fav_a, "fav_movement_type": fav_t,
        "market_acceleration_label": label,
    }
...
def build_quote_movement_package(fid, mk):
    ...
    quote_history = append_quote_history(get_existing_quote_history(fid), mk)
    quote_dynamics = analyze_quote_history(quote_history)
...
def calculate_visual_over_tier(structure_pack, market_pack, signal_pack):
    """
    Classificatore visivo bilanciato.

    Non decide se una partita entra o esce dalla dashboard.
    Serve solo a distinguere: base / strong / elite, riaccendendo
    i due palloni senza tornare ai filtri troppo restrittivi.
    """
    structure_pack = structure_pack or {}
    ...
    drop_type = str(market_pack.get("drop_type", "none")).strip().lower()
    drop_confirmed = bool(market_pack.get("drop_confirmed", False))
    value_left = str(market_pack.get("value_left", "unknown")).strip().lower()
    warning_flags = market_pack.get("warning_flags", []) or []

    over_level = int(signal_pack.get("over_level", 0) or 0)
    gold_ok = bool(signal_pack.get("gold_ok", False))
    boost_ok = bool(signal_pack.get("boost_ok", False))
    market_ok = bool(signal_pack.get("market_ok", False))
    inv_ok = bool(signal_pack.get("inv_ok", False))
    favorite_fertile = bool(signal_pack.get("favorite_fertile", False))
    combined_ht_avg = safe_float(signal_pack.get("combined_ht_avg", 0.0), 0.0)
    ...
    market_confirmed = bool(
        drop_confirmed
        or drop_type == "structural"
        or market_ok
        or inv_ok
    )

    # ELITE visiva: severa, ma non punitiva sulle favorite buone.
    if (
        over_level >= 2
        ...
        and (gold_ok or boost_ok or favorite_fertile or combined_ht_avg >= 1.10)
    ):
        return 2

    # STRONG visiva: base solida, anche senza bollino elite.
    if (
        over_level >= 1
...
#====================================
# BLOCCO 4 + BLOCCO 5
# ARAB SNIPER HYBRID CLASSIC ENGINE
# - Scocca V25: snapshot, GitHub, rotation, quote history, HTML
# - Cervello V24.1: score grezzi, diretti, leggibili
# - Obiettivo: tornare a una GOLD giocabile, non iper-filtrata
#====================================
...
def compute_drop_diff(fid, mk):
    """
    Drop della favorita 1/2 rispetto all'open snapshot.
    È volutamente semplice: se la favorita attuale è scesa, il mercato sta spingendo.
    """
    odds_memory = st.session_state.get("odds_memory", {}) or {}
    old_data = odds_memory.get(str(fid), {}) or {}
    if not isinstance(old_data, dict):
        return 0.0
...
def calc_one_sided_risk_classic(s_h, s_a):
    """
    Solo diagnostico: NON blocca la GOLD classic.
    Serve alla lente/HTML per capire se la partita è molto monolato.
    """
    home_attack = safe_float(s_h.get("avg_ft_scored_clean", s_h.get("avg_total", 0.0)), 0.0)
    away_attack = safe_float(s_a.get("avg_ft_scored_clean", s_a.get("avg_total", 0.0)), 0.0)
    home_ht = safe_float(s_h.get("avg_ht_scored_clean", s_h.get("avg_ht", 0.0)), 0.0)
    away_ht = safe_float(s_a.get("avg_ht_scored_clean", s_a.get("avg_ht", 0.0)), 0.0)

    risk = 0.0
    attack_gap = abs(home_attack - away_attack)
    ht_gap = abs(home_ht - away_ht)
    weak_attack = min(home_attack, away_attack)

    if attack_gap >= 1.00:
        risk += 1.20
    elif attack_gap >= 0.70:
        risk += 0.70
...
# SCORE V24.1 CLASSIC
# ------------------------------------------------------------
def score_pt_signal_v24(mk, s_h, s_a, combined_ht_avg):
    score = 0.0

    score += band_score(combined_ht_avg, 1.12, 1.70, 1.05, 1.90, core_pts=1.5, soft_pts=0.8)

    if safe_float(s_h.get("avg_ht"), 0.0) >= 1.10 and safe_float(s_a.get("avg_ht"), 0.0) >= 1.10:
        score += 1.6
    elif (
        safe_float(s_h.get("avg_ht"), 0.0) >= 1.25 and safe_float(s_a.get("avg_ht"), 0.0) >= 0.95
    ) or (
        safe_float(s_a.get("avg_ht"), 0.0) >= 1.25 and safe_float(s_h.get("avg_ht"), 0.0) >= 0.95
    ):
        score += 1.0

    score += symmetry_bonus(s_h.get("avg_ht", 0.0), s_a.get("avg_ht", 0.0), tight=0.20, medium=0.40)
    score += band_score(mk.get("o05ht", 0.0), 1.20, 1.40, 1.15, 1.48, core_pts=1.6, soft_pts=0.7)
    score += band_score(mk.get("o15ht", 0.0), 2.00, 3.60, 1.80, 4.20, core_pts=0.8, soft_pts=0.3)

    if bool(s_h.get("last_2h_zero", False)) or bool(s_a.get("last_2h_zero", False)):
        score += 0.8

    if safe_float(s_h.get("avg_total"), 0.0) >= 1.20 and safe_float(s_a.get("avg_total"), 0.0) >= 1.20:
        score += 0.5

    return round3(score)


def score_over_signal_v24(mk, s_h, s_a, combined_ht_avg, fav, drop_diff):
    score = 0.0
    h_tot = safe_float(s_h.get("avg_total"), 0.0)
    a_tot = safe_float(s_a.get("avg_total"), 0.0)

    if h_tot >= 1.55 and a_tot >= 1.55:
        score += 2.2
    elif h_tot >= 1.45 and a_tot >= 1.45:
        score += 1.4
    elif (h_tot >= 1.80 and a_tot >= 1.20) or (a_tot >= 1.80 and h_tot >= 1.20):
        score += 1.0

    score += symmetry_bonus(h_tot, a_tot, tight=0.28, medium=0.50)
    score += band_score(mk.get("o25", 0.0), 1.51, 2.37, 1.40, 2.55, core_pts=1.8, soft_pts=0.8)
...
def score_boost_signal_v24(mk, s_h, s_a, pt_score, over_score, drop_diff, combined_ht_avg):
    score = 0.0
    h_ht = safe_float(s_h.get("avg_ht"), 0.0)
    a_ht = safe_float(s_a.get("avg_ht"), 0.0)
    h_tot = safe_float(s_h.get("avg_total"), 0.0)
    a_tot = safe_float(s_a.get("avg_total"), 0.0)
    ...
    if (h_ht >= 1.30 and a_ht >= 1.00) or (a_ht >= 1.30 and h_ht >= 1.00):
        score += 0.55
    elif h_ht >= 1.15 and a_ht >= 1.15:
        score += 0.35

    if h_tot >= 1.65 and a_tot >= 1.65:
        score += 0.55
    elif (h_tot >= 1.95 and a_tot >= 1.35) or (a_tot >= 1.95 and h_tot >= 1.35):
        score += 0.25

    if 1.60 <= safe_float(mk.get("o25"), 0.0) <= 2.12 and 1.22 <= safe_float(mk.get("o05ht"), 0.0) <= 1.36:
        score += 0.55
    elif 1.55 <= safe_float(mk.get("o25"), 0.0) <= 2.20 and 1.20 <= safe_float(mk.get("o05ht"), 0.0) <= 1.38:
        score += 0.20
...
def score_gold_signal_v24(mk, s_h, s_a, pt_score, over_score, boost_score, fav, drop_diff, is_gold_zone, combined_ht_avg):
    score = 0.0
    h_tot = safe_float(s_h.get("avg_total"), 0.0)
    a_tot = safe_float(s_a.get("avg_total"), 0.0)
    ...
    if combined_ht_avg >= 1.18 and h_tot >= 1.55 and a_tot >= 1.50:
        score += 0.45
...
# ------------------------------------------------------------
# PROFILI COMPATIBILI CON DETAILS / HTML V25
# ------------------------------------------------------------
def build_match_structure_profile(mk, s_h, s_a, market_pack=None, quote_pack=None):
    h_tot = safe_float(s_h.get("avg_total", 0.0), 0.0)
    a_tot = safe_float(s_a.get("avg_total", 0.0), 0.0)
    h_ht = safe_float(s_h.get("avg_ht", 0.0), 0.0)
    a_ht = safe_float(s_a.get("avg_ht", 0.0), 0.0)

    h_ft_scored = safe_float(s_h.get("avg_ft_scored_clean", h_tot), 0.0)
    a_ft_scored = safe_float(s_a.get("avg_ft_scored_clean", a_tot), 0.0)
    h_ft_conc = safe_float(s_h.get("avg_ft_conceded_clean", 0.0), 0.0)
    a_ft_conc = safe_float(s_a.get("avg_ft_conceded_clean", 0.0), 0.0)
    h_ht_scored = safe_float(s_h.get("avg_ht_scored_clean", h_ht), 0.0)
    a_ht_scored = safe_float(s_a.get("avg_ht_scored_clean", a_ht), 0.0)

    combined_ht_clean = round3((safe_float(s_h.get("avg_ht_clean", h_ht), 0.0) + safe_float(s_a.get("avg_ht_clean", a_ht), 0.0)) / 2)
    combined_ft_clean = round3((safe_float(s_h.get("avg_total_clean", h_tot), 0.0) + safe_float(s_a.get("avg_total_clean", a_tot), 0.0)) / 2)
    combined_ht_scored_clean = round3((h_ht_scored + a_ht_scored) / 2)
    cross_home_clean = round3(h_ft_scored + a_ft_conc)
    cross_away_clean = round3(a_ft_scored + h_ft_conc)

    fav = min(safe_float(mk.get("q1"), 0.0), safe_float(mk.get("q2"), 0.0))
    one_sided_risk = calc_one_sided_risk_classic(s_h, s_a)
    bilateral_ft = bool(h_tot >= 1.00 and a_tot >= 1.00)
    bilateral_ht = bool(h_ht >= 0.75 and a_ht >= 0.75)

    structure_score = 0.0
    if combined_ft_clean >= 1.55:
        structure_score += 0.8
    elif combined_ft_clean >= 1.35:
        structure_score += 0.4
    if combined_ht_clean >= 1.00:
        structure_score += 0.55
    elif combined_ht_clean >= 0.85:
    ...
    match_profile = "v24_classic"
    if combined_ht_clean >= 1.12 and combined_ft_clean >= 1.60:
        match_profile = "classic_fast_over"
    elif 1.50 <= fav <= 1.87 and combined_ft_clean >= 1.50:
        match_profile = "classic_favorite_over"
    elif one_sided_risk >= 1.40:
        match_profile = "classic_one_sided_watch"

    return {
        "match_profile": match_profile,
        "structure_score": round3(structure_score),
        "structure_grade": "high" if structure_score >= 1.65 else "medium" if structure_score >= 0.95 else "low",
        "combined_ht_clean": combined_ht_clean,
        "combined_ft_clean": combined_ft_clean,
        "combined_ht_scored_clean": combined_ht_scored_clean,
        "cross_home_clean": cross_home_clean,
        ...
        "fav_quote": round3(fav),
        "fav_zone": classify_favorite_zone(fav),
        "combined_ht_dirty": round3((h_ht + a_ht) / 2),
        "combined_ft_dirty": round3((h_tot + a_tot) / 2),
        "cross_home_dirty": cross_home_clean,
        "cross_away_dirty": cross_away_clean,
    }
...
def build_structure_debug_summary(structure_pack):
    if not structure_pack:
        return {}
    return {
        "match_profile": structure_pack.get("match_profile", "v24_classic"),
        "structure_score": structure_pack.get("structure_score", 0.0),
        "structure_grade": structure_pack.get("structure_grade", "medium"),
...
def build_market_pack_classic(mk, quote_pack, structure_pack):
    """
    Mercato usato come informazione, non come padrone della decisione.
    Manteniamo i campi V25 per HTML/lente/refiner.
    """
    drop_quality = classify_drop_quality(quote_pack)
    pressure = detect_primary_market_pressure(quote_pack)
    fav = safe_float(structure_pack.get("fav_quote", 0.0), 0.0)
    o25 = safe_float(mk.get("o25", 0.0), 0.0)
    o05 = safe_float(mk.get("o05ht", 0.0), 0.0)
    ...
    if 1.50 <= fav <= 1.87:
        ...
        positive_flags.append("classic_fav_fertile")
    if 1.45 <= o25 <= 2.20:
        coherence_score += 0.65
        positive_flags.append("classic_o25_range")
    if 1.18 <= o05 <= 1.42:
        coherence_score += 0.45
        positive_flags.append("classic_ht_market_range")
    if drop_quality.get("confirmed") or pressure.get("leader") in ("mixed", "1x2", "o25"):
        coherence_score += 0.35
        positive_flags.append("classic_market_pressure")

    if o25 > 2.55 and o25 != 0:
        warning_flags.append("classic_o25_too_high")
    if fav < 1.25 and o25 <= 1.55:
        warning_flags.append("classic_ultra_fav_compressed")

    return {
        "market_profile": "classic_market",
        "coherence_score": round3(coherence_score),
        "coherence_level": "high" if coherence_score >= 1.55 else "medium" if coherence_score >= 0.85 else "low",
        "dislocation_score": 0.0,
        "dislocation_level": "low",
        "leading_market": pressure.get("leader", "none"),
        "lagging_market": "none",
        "pressure_strength": pressure.get("strength", "none"),
        "drop_type": drop_quality.get("drop_type", "none"),
        "drop_strength": drop_quality.get("drop_strength", "none"),
        "drop_confirmed": bool(drop_quality.get("confirmed", False)),
        "value_left": drop_quality.get("value_left", "unknown"),
        "positive_flags": positive_flags,
        "warning_flags": warning_flags,
        "baseline": build_market_baseline(mk),
        "struct_snapshot": {},
        ...
        "margin_curr": 0.0,
        "margin_delta": 0.0,
        "fav_fair_prob_open": 0.0,
        "fav_fair_prob_curr": 0.0,
        "fav_fair_prob_delta": 0.0,
        "fav_fair_curr": 0.0,
        "home_ft_stdev": 0.0,
        "away_ft_stdev": 0.0,
        "home_scoring_regularity": 0.0,
        "away_scoring_regularity": 0.0,
    }


# ------------------------------------------------------------
# BUILD SIGNAL PACKAGE HYBRID CLASSIC
# ------------------------------------------------------------
def build_signal_package(fid, mk, s_h, s_a):
    ...
    combined_ht_avg = round3((safe_float(s_h.get("avg_ht", 0.0), 0.0) + safe_float(s_a.get("avg_ht", 0.0), 0.0)) / 2)

    pt_score = score_pt_signal_v24(mk, s_h, s_a, combined_ht_avg)
    over_score = score_over_signal_v24(mk, s_h, s_a, combined_ht_avg, fav, drop_diff)
    boost_score = score_boost_signal_v24(mk, s_h, s_a, pt_score, over_score, drop_diff, combined_ht_avg)
    gold_score = score_gold_signal_v24(mk, s_h, s_a, pt_score, over_score, boost_score, fav, drop_diff, is_gold_zone, combined_ht_avg)

    structure_pack = build_match_structure_profile(mk, s_h, s_a, quote_pack=quote_pack)
    market_pack = build_market_pack_classic(mk, quote_pack, structure_pack)

    try:
        lambda_pack = estimate_match_lambdas(s_h, s_a)
    except Exception:
        lambda_pack = {"lam_home_ft": 0.0, "lam_away_ft": 0.0, "lam_home_ht": 0.0, "lam_away_ht": 0.0, "ft_sd_avg": 0.0, "ht_sd_avg": 0.0}

    try:
        tempo_pack = classify_match_tempo(s_h, s_a)
    except Exception:
        tempo_pack = {"tempo_tag": "CLASSIC", "early_index": combined_ht_avg}
    ...
    has_drop_1x2 = (
        ...
        or (q2_move_data.get("dir") == "down" and safe_float(q2_move_data.get("abs_diff", 0.0), 0.0) >= 0.06)
    )
    has_drop_o25 = o25_move_data.get("dir") == "down" and safe_float(o25_move_data.get("abs_diff", 0.0), 0.0) >= 0.05
    inv_ok = bool(quote_pack.get("INVERSION", False))
    drop_confirmed = bool(market_pack.get("drop_confirmed", False))

    drop_strength_score = 0
    if drop_confirmed:
        drop_strength_score = 3
    if drop_diff >= 0.15:
        drop_strength_score = max(drop_strength_score, 3)
    elif drop_diff >= 0.10:
        drop_strength_score = max(drop_strength_score, 2)
    elif drop_diff >= 0.05 or has_drop_1x2 or has_drop_o25:
        drop_strength_score = max(drop_strength_score, 1)

    drop_visual_level = "strong" if drop_strength_score >= 3 else "medium" if drop_strength_score == 2 else "light" if drop_strength_score == 1 else "none"
    drop_medium_or_strong = drop_visual_level in ("medium", "strong")

    # Gate CLASSIC: pochi, chiari, non punitivi.
    classic_base = bool(
        favorite_fertile
        and 1.40 <= o25 <= 2.55
        and 1.15 <= o05 <= 1.48
    ...
    classic_core = bool(
        ...
        and h_tot >= 1.45
        and a_tot >= 1.35
        and combined_ht_avg >= 1.10
    )

    pt_ok = bool(pt_score >= 4.10 and combined_ht_avg >= 1.05)
    over_ok = bool(over_score >= 4.00)
    boost_ok = bool(
        boost_score >= 5.85
        and pt_score >= 4.00
        and over_score >= 4.15
    ...
    gold_ok = bool(
        (
            gold_score >= 6.75
            and boost_score >= 5.85
            and pt_score >= 4.00
            and over_score >= 4.15
            and is_gold_zone
            and (classic_base or drop_diff >= 0.05)
        )
        or
        (
            classic_core
            and over_score >= 4.00
            and pt_score >= 3.85
            and (drop_diff >= 0.05 or has_drop_1x2 or has_drop_o25 or inv_ok or h_tot >= 1.65 or a_tot >= 1.65)
        )
    )

    market_ok = bool(drop_confirmed or has_drop_1x2 or has_drop_o25 or inv_ok)
    probe_ok = bool((favorite_fertile and h_tot >= 1.00 and a_tot >= 1.00) or (classic_base and over_score >= 3.40))

    if over_score >= 4.80 or boost_ok:
        over_level = 3
    elif over_score >= 4.30:
        over_level = 2
    elif over_score >= 3.80:
        over_level = 1
    else:
        over_level = 0

    tags = []
    internal_labels = ["ENGINE_V24_CLASSIC"]
    ...
    if classic_base:
        tags.append("CLASSIC")
        internal_labels.append("CLASSIC_BASE")
    if classic_core:
        internal_labels.append("CLASSIC_CORE")
    if favorite_fertile:
        internal_labels.append("FAV_FERTILE")
    if market_ok:
    ...
    visual_signal_pack = {
        ...
        "probe_ok": probe_ok,
        "inv_ok": inv_ok,
        "favorite_fertile": favorite_fertile,
        "favorite_core": favorite_core,
        "combined_ht_avg": combined_ht_avg,
    }
    visual_over_tier = calculate_visual_over_tier(structure_pack, market_pack, visual_signal_pack)

    # Segnale ⚽⚽: resta visivo, ma ora viene assegnato solo quando
    # la lettura classica è confermata da struttura + mercato.
    gg_ht_sniper = bool(
        visual_over_tier >= 2
        or (gold_ok and over_level >= 2 and combined_ht_avg >= 1.10 and (favorite_fertile or inv_ok))
        or (market_ok and drop_medium_or_strong and combined_ht_avg >= 1.05 and over_level >= 1)
    )
    if gg_ht_sniper:
        tags.append("⚽⚽")
    ...
    strong_tag_count = sum(1 for t in tags if t in ("GOLD", "BOOST", "OVER", "PT", "MARKET", "PROBE"))
    max_score = round3(max(pt_score, over_score, boost_score, gold_score))

    if visual_over_tier >= 2:
        signal_stability = "Alta"
        signal_summary = "ELITE VISIVA: struttura + mercato confermati"
    elif gold_ok:
        signal_stability = "Alta"
        signal_summary = "GOLD CLASSIC: favorita fertile + base goal/HT"
    elif boost_ok:
        signal_stability = "Alta"
        signal_summary = "BOOST CLASSIC: PT + OVER allineati"
    elif visual_over_tier == 1:
        signal_stability = "Media"
        signal_summary = "STRONG VISIVO: base Over valida"
    elif over_ok:
        signal_stability = "Media"
        signal_summary = "OVER CLASSIC: base FT concreta"
    elif pt_ok:
        signal_stability = "Media"
        signal_summary = "PT CLASSIC: pressione primo tempo"
    elif market_ok:
        signal_stability = "Media"
        signal_summary = "Market/Drop da monitorare"
    else:
        signal_stability = "Speculativa"
        signal_summary = "Probe classic"
    ...
    scores = {
        ...
        "edge_o05ht": 0.0,
        "edge_o15ht": 0.0,
        "edge_level_o25": "CLASSIC",
        "edge_level_o05ht": "CLASSIC",
        "edge_level_o15ht": "CLASSIC",
        "p_model_o25": 0.0,
        "p_market_o25": 0.0,
        "p_model_o05ht": 0.0,
        "p_market_o05ht": 0.0,
    ...
    quote_dynamics = quote_pack.get("QUOTE_DYNAMICS", {}) or {}
    ...
    return {
        ...
        "fav_quote": round3(fav),
        "is_gold_zone": bool(favorite_fertile),
        "is_gold_signal": bool(gold_ok),
        "fav_fertile_zone": bool(favorite_fertile),
        "strong_tag_count": strong_tag_count,
        "primary_signal_count": strong_tag_count,
        "quote_pack": quote_pack,
        "market_pack": market_pack,
        "structure_pack": structure_pack,
        "internal_labels": internal_labels,
        "visual_over_tier": visual_over_tier,
        "over_level": over_level,
        "has_inversion": inv_ok,
        "lambda_pack": lambda_pack,
        "tempo_pack": tempo_pack,
        "tempo_tag": tempo_pack.get("tempo_tag", "CLASSIC"),
        "early_index": tempo_pack.get("early_index", combined_ht_avg),
        "quote_history": quote_pack.get("QUOTE_HISTORY", []),
        "quote_dynamics": quote_dynamics,
        "market_acceleration_label": quote_dynamics.get("market_acceleration_label", "NO_HISTORY"),
        "edge_o25": 0.0,
        "edge_o05ht": 0.0,
        "edge_o15ht": 0.0,
        "edge_level_o25": "CLASSIC",
        ...
        "base_ft_ok": over_ok,
        "base_ht_ok": pt_ok,
        "fav_native_ok": favorite_fertile,
...
def should_keep_match(signal_pack):
    """
    Filtro finale snello.
    Non fa più il giudice matematico: tiene ciò che il motore classic ha letto.
    """
    ...
    max_score = safe_float(scores.get("max", 0.0), 0.0)
    fav = safe_float(signal_pack.get("fav_quote", 0.0), 0.0)
    favorite_fertile = bool(1.50 <= fav <= 1.87)

    if any(t in tags for t in ("GOLD", "BOOST", "OVER", "PT", "MARKET")):
        return True

    if "CLASSIC" in tags and max_score >= 3.40:
        return True

    if "PROBE" in tags and favorite_fertile and max_score >= 3.25:
        return True
...
def build_signal_debug_summary(signal_pack):
    if not signal_pack:
        return {}
    return {
        "engine": "V24_CLASSIC_ON_V25_SHELL",
        ...
        "visual_over_tier": signal_pack.get("visual_over_tier", 0),
        ...
        "market_acceleration_label": signal_pack.get("market_acceleration_label", "NO_HISTORY"),
        "quote_dynamics": signal_pack.get("quote_dynamics", {}),
    }



# ============================================================
# ARABSNIPER AUDIT-DRIVEN REFINER — FIX 2026-05-04
# Strato finale: MARKET validatore, DROP non cieco, FT/HT separati.
# Campi generati per HTML:
...
def asr_bool(v):
    ...
    if isinstance(v, (int, float)):
        return v != 0
    return str(v).strip().lower() in ("1", "true", "yes", "y", "si", "sì", "ok")

def asr_text(row, *keys):
    parts = []
    for k in keys:
        v = row.get(k)
        if v is not None:
            parts.append(str(v))
    legacy = row.get("legacy_tags") or row.get("LEGACY_TAGS") or []
    if isinstance(legacy, list):
        parts.append(" ".join(map(str, legacy)))
    else:
        parts.append(str(legacy))
    return " ".join(parts).upper()

def asr_over_level(row):
    """
    Legge il livello OVER reale già prodotto dallo script.
    Fix: prima dei testi deve leggere OVER_LEVEL / over_level.
    """
    # 1) Campo numerico già presente nel JSON finale
    for key in ("OVER_LEVEL", "over_level"):
        if key in row and row.get(key) not in (None, ""):
            lvl = int(safe_float(row.get(key), 0))
            if lvl in (0, 1, 2, 3):
                return lvl

    # 2) Canonical eventuale
    canonical = row.get("canonical") or row.get("CANONICAL") or {}
    if isinstance(canonical, dict) and canonical.get("over_level") is not None:
        lvl = int(safe_float(canonical.get("over_level"), 0))
        if lvl in (0, 1, 2, 3):
            return lvl

    # 3) Testi / tag legacy
    primary = str(row.get("primary_signal", row.get("PRIMARY_SIGNAL", ""))).upper()
    info = asr_text(row, "info_raw", "INFO", "Info", "primary_signal", "PRIMARY_SIGNAL")
    if "OVER_L3" in primary or "OVER_L3" in info:
        return 3
    if "OVER_L2" in primary or "OVER_L2" in info:
        return 2
    if "OVER_L1" in primary or "OVER_L1" in info:
        return 1
    if "STRONG_OVER" in info or "BOOST" in info:
        return 3
    if "OVER" in info:
        return 1
    return 0
...
def asr_flags(row):
    canonical = row.get("canonical") or row.get("CANONICAL") or {}
    txt = asr_text(row, "info_raw", "INFO", "Info", "primary_signal", "PRIMARY_SIGNAL")
    def cflag(name):
        if isinstance(canonical, dict) and name in canonical:
            return asr_bool(canonical.get(name))
        return False
    over_level = asr_over_level(row)
    return {
        "gold": cflag("gold") or "GOLD" in txt,
        "market": cflag("market") or "MARKET" in txt,
        "drop": cflag("drop") or "DROP" in txt or safe_float(row.get("drop_diff", row.get("DROP_DIFF", 0)), 0) > 0,
        "inv": cflag("inv") or asr_bool(row.get("has_inversion", row.get("INVERSION", False))) or "INV" in txt,
        "probe": cflag("probe") or "PROBE" in txt,
        "pt": cflag("pt") or " PT" in f" {txt} " or "PTGG" in txt or "PTO15" in txt,
        "ball2": "⚽⚽" in str(row.get("info_raw", row.get("INFO", row.get("Info", "")))) or "⚽⚽" in txt,
        "over": over_level > 0,
    }

def asr_classify_drop(row):
    diff = abs(safe_float(row.get("drop_diff", row.get("DROP_DIFF", 0.0)), 0.0))
    if diff <= 0:
        cls, label, weight = "none", "", 0.0
    elif diff < 0.05:
        cls, label, weight = "soft", "DROP_SOFT", 0.30
    elif diff < 0.10:
        cls, label, weight = "mid", "DROP_MID", 0.45
    elif diff < 0.20:
        cls, label, weight = "big", "DROP_BIG", 0.10
    else:
        cls, label, weight = "hard", "DROP_HARD", -0.20
    return {"drop_value": round3(diff), "drop_class": cls, "drop_label": label, "drop_weight": weight}

def asr_pick(row, *keys):
    for k in keys:
        if k in row and row.get(k) not in (None, ""):
            return row.get(k)
    return 0

def asr_odds_context(row):
    q1_open = safe_float(asr_pick(row, "q1_open", "Q1_OPEN", "home_open", "odd_1_open"), 0)
    qx_open = safe_float(asr_pick(row, "qx_open", "QX_OPEN", "draw_open", "odd_x_open"), 0)
    q2_open = safe_float(asr_pick(row, "q2_open", "Q2_OPEN", "away_open", "odd_2_open"), 0)

    q1_curr = safe_float(asr_pick(row, "q1_curr", "Q1_CURR", "Q1", "home_curr", "odd_1_curr"), 0)
    qx_curr = safe_float(asr_pick(row, "qx_curr", "QX_CURR", "QX", "draw_curr", "odd_x_curr"), 0)
    q2_curr = safe_float(asr_pick(row, "q2_curr", "Q2_CURR", "Q2", "away_curr", "odd_2_curr"), 0)

    o25_open = safe_float(asr_pick(row, "o25_open", "O25_OPEN"), 0)
    o25_curr = safe_float(asr_pick(row, "o25_curr", "O25_CURR", "O2.5"), 0)

    o05ht_open = safe_float(asr_pick(row, "o05ht_open", "O05HT_OPEN"), 0)
    o05ht_curr = safe_float(asr_pick(row, "o05ht_curr", "O05HT_CURR", "O0.5H"), 0)

    o15ht_open = safe_float(asr_pick(row, "o15ht_open", "O15HT_OPEN"), 0)
    o15ht_curr = safe_float(asr_pick(row, "o15ht_curr", "O15HT_CURR", "O1.5H"), 0)

    fav_open = min([x for x in (q1_open, q2_open) if x > 0], default=0)
    fav_curr = min([x for x in (q1_curr, q2_curr) if x > 0], default=0)
    ...
    return {
        "q1_open": q1_open, "qx_open": qx_open, "q2_open": q2_open,
        "q1_curr": q1_curr, "qx_curr": qx_curr, "q2_curr": q2_curr,
        "o25_open": o25_open, "o25_curr": o25_curr,
        "o05ht_open": o05ht_open, "o05ht_curr": o05ht_curr,
        "o15ht_open": o15ht_open, "o15ht_curr": o15ht_curr,
        "fav_open": fav_open, "fav_curr": fav_curr, "fav_band": fav_band,
    }

def asr_score_ft(row, flags, drop, odds):
    over_level = asr_over_level(row)
    score = 0.0
    ...
    score += safe_float(drop.get("drop_weight"), 0.0)

    if flags["probe"] and not flags["market"]:
        score -= 0.20
    ...
    # dagli audit: drop hard senza market/inv spesso è trappola
    if drop["drop_class"] == "hard" and not flags["market"] and not flags["inv"]:
        score -= 0.50

    return round3(max(score, 0.0))

def asr_score_ht(row, flags, drop, odds):
    over_level = asr_over_level(row)
    score = 0.0
    ...
    if drop["drop_class"] in ("soft", "mid"):
        score += 0.15
    elif drop["drop_class"] == "hard" and not flags["market"]:
        score -= 0.20

    o05ht = safe_float(odds.get("o05ht_curr"), 0)
    o15ht = safe_float(odds.get("o15ht_curr"), 0)
...
def refine_arabsniper_signal(row):
    ...
    if flags["market"]:
        tags.append("MARKET")
    if flags["drop"]:
        tags.append(drop["drop_label"] or "DROP")
    if flags["inv"]:
        tags.append("INV")
    if flags["probe"]:
        tags.append("PROBE")
...
def row_fixture_datetime_rome(row: dict):
    """
    Ricostruisce data+ora della riga in Europe/Rome.
    Serve per evitare che il fast scan tenga in HTML partite già finite.
    """
    try:
        d = str((row or {}).get("Data", "")).strip()
        h = str((row or {}).get("Ora", "")).strip()
        if not d or not h:
            return None

        # Normalizza "16:00:00" -> "16:00"
        h = h[:5]
...
def row_age_minutes(row: dict) -> float:
    dt = row_fixture_datetime_rome(row)
    if not dt:
        return -999999.0

    now_dt = now_rome()
    if getattr(now_dt, "tzinfo", None) is None and getattr(dt, "tzinfo", None) is not None:
        now_dt = now_dt.replace(tzinfo=dt.tzinfo)

    return (now_dt - dt).total_seconds() / 60.0


def is_row_in_live_hold_window(row: dict) -> bool:
    """
    True solo se la partita è iniziata da poco ed è ancora sensato tenerla.
    Esempio: scan alle 16:00 -> partita delle 14:30 resta; partita delle 11:00 sparisce.
    """
    age = row_age_minutes(row)
    return age >= -PRE_KICKOFF_GRACE_MINUTES and age <= LIVE_HOLD_MINUTES


def should_publish_day_row(row: dict, target_date: str) -> bool:
    """
    Regola unica per data_dayX.json e data.json.
    - mai righe di altra data
    - mai stale
    - live_hold solo dentro finestra live
    - se manca due volte, via comunque
    """
    if str((row or {}).get("Data", "")).strip() != str(target_date).strip():
        return False

    status = str((row or {}).get("status", "")).lower().strip()
    missing_count = int((row or {}).get("missing_count", 0) or 0)
    live_hold = bool((row or {}).get("LIVE_HOLD", False))

    if status in ("stale", "expired", "finished", "old"):
        return False

    if live_hold:
        if missing_count >= 2:
            return False
        if not is_row_in_live_hold_window(row):
            return False
...
def build_day_results(day_num):
    ...
    results = [
        ...
        if should_publish_day_row(r, target_date)
    ]
...
def mark_row_as_stale(row: dict) -> dict:
    ...
    old_missing = int(stale.get("missing_count", 0) or 0)
    stale["missing_count"] = old_missing + 1

    if is_row_in_live_hold_window(stale) and stale["missing_count"] <= 1:
        stale["status"] = "live_hold"
        stale["LIVE_HOLD"] = True
        summary = str(stale.get("SIGNAL_SUMMARY", "")).strip()
        note = "Live hold: match iniziato/non aggiornabile"
        stale["SIGNAL_SUMMARY"] = summary if note in summary else (summary + f" | {note}" if summary else note)
    else:
        stale["status"] = "expired"
        stale["LIVE_HOLD"] = False
...
def merge_day_rows(old_rows: list, new_rows: list) -> list:
    ...
    for fid, old_row in old_map.items():
        if fid not in merged_map:
            stale_row = mark_row_as_stale(old_row)
            # Se è già fuori finestra live, non va più portata nel day file.
            if should_publish_day_row(stale_row, target_date):
                merged_map[fid] = stale_row
            ...
            for i, f in enumerate(day_fx):
                ...
                try:
                    ...
                    row = {
                        ...
                        "DROP_VISUAL_LEVEL": signal_pack.get("drop_visual_level", "none"),
                        "QUOTE_HISTORY": quote_pack.get("QUOTE_HISTORY", []),
                        "QUOTE_DYNAMICS": quote_pack.get("QUOTE_DYNAMICS", {}),
                        "MARKET_ACCELERATION_LABEL": signal_pack.get("market_acceleration_label", "NO_HISTORY"),
                        "O25_VELOCITY": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o25_velocity", 0.0),
                        "O25_ACCELERATION": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o25_acceleration", 0.0),
                        "O05HT_VELOCITY": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o05ht_velocity", 0.0),
                        "O05HT_ACCELERATION": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o05ht_acceleration", 0.0),
                        "O15HT_VELOCITY": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o15ht_velocity", 0.0),
                        "O15HT_ACCELERATION": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("o15ht_acceleration", 0.0),
                        "FAV_VELOCITY": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("fav_velocity", 0.0),
                        "FAV_ACCELERATION": (quote_pack.get("QUOTE_DYNAMICS", {}) or {}).get("fav_acceleration", 0.0),
                    ...
                    # ----------------------------------
                    # AUDIT-DRIVEN REFINER 2026-05-04
                    # Aggiunge campi smart per HTML senza rompere i campi esistenti.
                    # ----------------------------------
                    try:
                        row.update(refine_arabsniper_signal(row))
                    except Exception as e_refiner:
                        print(f"⚠️ Refiner ArabSniper non applicato su fixture {fid}: {e_refiner}", flush=True)
...
# UI SIDEBAR
# ------------------------------------------
st.sidebar.header("👑 Arab Sniper Hybrid Classic")
...
# --- PATCH 2026-05-04: override detect_over_level ---
def detect_over_level(row):
    """
    Rileva livello OVER da campi nuovi/vecchi.
    Fix 2026-05-04:
    - legge prima OVER_LEVEL / over_level già prodotto da 3appdays.py
    - poi canonical
    - poi primary_signal / Info / legacy
    """
    # 1) Campi già presenti nel JSON finale
    for key in ("OVER_LEVEL", "over_level", "OVER_LEVEL_REFINED", "over_level_refined"):
        if key in row and row.get(key) not in (None, ""):
            lvl = int(_safe_float(row.get(key), 0))
            if lvl in (0, 1, 2, 3):
                return lvl
    ...
    if isinstance(canonical, dict):
        lvl = canonical.get("over_level")
        if lvl is not None:
            lvl = int(_safe_float(lvl, 0))
            if lvl in (0, 1, 2, 3):
                return lvl

    # 3) Testi e tag
    primary = str(row.get("primary_signal", "") or row.get("PRIMARY_SIGNAL", "")).upper()
    info = str(row.get("Info", "") or row.get("info_raw", "") or row.get("INFO_RAW", "")).upper()
    ...
    legacy = row.get("legacy_tags") or row.get("LEGACY_TAGS") or []
    legacy_txt = " ".join(map(str, legacy)).upper() if isinstance(legacy, list) else str(legacy).upper()
    all_txt = f"{primary} {info} {legacy_txt}"

    if "BOOST" in all_txt or "STRONG_OVER" in all_txt:
        return 3
    if "OVER" in all_txt:
        return 1
