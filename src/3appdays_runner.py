#!/usr/bin/env python3
"""Runner separato per ArabSniper Quote Engine V1.1.1.
...
def build_command(mode: str, github: bool = False, days: int = 5) -> list[str]:
    python_exe = sys.executable or "python"
    command = [python_exe, str(ENGINE_FILE)]
    if mode == "fast":
...
def run(mode: str, github: bool = False, days: int = 5) -> int:
    if not ENGINE_FILE.exists():
        log(f"❌ Motore non trovato: {ENGINE_FILE}")
        return 2

    command = build_command(mode, github=github, days=days)
...
def main() -> None:
    args = parse_args()
    mode = "fast" if args.fast else "night" if args.night else "catalog"
    raise SystemExit(run(mode, github=args.github, days=max(1, min(args.days, 7))))
