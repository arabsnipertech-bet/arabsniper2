    ...
    for response_row in odds_payload.get("response", []) or []:
        for bookmaker in response_row.get("bookmakers", []) or []:
            ...
            for bet in bookmaker.get("bets", []) or []:
                ...
                # Totali goal: scarta corner/cartellini/team totals
                if any(token in bet_name for token in ("corner", "card", "booking", "team total")):
                    continue
                if not any(token in bet_name for token in ("total", "over under", "goals")) and bet_id != 5:
                    continue

                over25 = _find_odd(values, ("over 2.5", "over 2,5"))
                under25 = _find_odd(values, ("under 2.5", "under 2,5"))
                if not first_half and over25 > 1 and under25 > 1:
                    book_record["markets"][MARKET_O25] = {"over": over25, "under": under25}

                over15 = _find_odd(values, ("over 1.5", "over 1,5"))
                under15 = _find_odd(values, ("under 1.5", "under 1,5"))
                if first_half and over15 > 1 and under15 > 1:
                    book_record["markets"][MARKET_O15HT] = {"over": over15, "under": under15}
